...
// Prober creates a UDP connection with the address and sends and receives
...
// NewProber creates a prober that sends and receives UDP messages.
...
// Probe exchanges the messages from client to the server and vice-versa
...
// dialUDP establishes a UDP connection between the client and the server.
func (p *Prober) dialUDP(ctx context.Context) (*net.UDPConn, error) {
	dialer := net.Dialer{Timeout: p.timeout}
	conn, err := dialer.DialContext(ctx, "udp", p.address)
	if err != nil {
		return nil, err
	}
...
// ProbeResult is the result for UDP probe.
